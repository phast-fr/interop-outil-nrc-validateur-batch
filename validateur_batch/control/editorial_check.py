...
def _check_or4(df: pd.DataFrame) -> pd.DataFrame:
    ...
    EXCEPTIONS = ["extern", "extrém", "extrem", "excré", "mineur", "micro", "mini", "mitral", "extra", "except", "extrins"]
...
def _check_bs8(df: pd.DataFrame, pt: pd.Series, syn: pd.Series) -> pd.DataFrame:
    ...
    idx = df.loc[pt
                 ...
                 & (~df.loc[:, "term"].str.contains(r"(?:apex|pointe|bout|cime|sommet)", case=False))].index # noqa
